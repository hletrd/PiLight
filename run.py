...
def logLED(inp):
	...
	try:
		sumLED = math.pow(inp, 3)/math.pow(sumLED, 3)
	except:
		sumLED = 0
	return inp * sumLED
...
def calcR(rawR):
	...
	return int(rawR / 255.0 * rMax / 100 * colorTemptoRGB(tNow)['r']/255*18000)

def calcG(rawG):
	rawG = logLED(1.0*rawG)
	return int(rawG / 255.0 * gMax / 100 * colorTemptoRGB(tNow)['g']/255*18000)

def calcB(rawB):
	rawB = logLED(1.0*rawB)
	return int(rawB / 255.0 * bMax / 100 * colorTemptoRGB(tNow)['b']/255*18000)
